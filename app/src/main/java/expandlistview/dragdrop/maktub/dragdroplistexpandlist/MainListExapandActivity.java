package expandlistview.dragdrop.maktub.dragdroplistexpandlist;
...
import expandlistview.dragdrop.maktub.dragdroplistexpandlist.fragment.DragDropListFragment;
import expandlistview.dragdrop.maktub.dragdroplistexpandlist.fragment.ExampleExpandableDataProviderFragment;
import expandlistview.dragdrop.maktub.dragdroplistexpandlist.fragment.ExpandableItemPinnedMessageDialogFragment;
import expandlistview.dragdrop.maktub.dragdroplistexpandlist.provide.AbstractExpandableDataProvider;

public class MainListExapandActivity extends AppCompatActivity implements ExpandableItemPinnedMessageDialogFragment.EventListener {
    private static final String FRAGMENT_TAG_ITEM_PINNED_DIALOG = "item pinned dialog";
    private String FRAGMENT_LIST_VIEW = "DragDropListFragment";
    private String FRAGMENT_TAG_DATA_PROVIDER = "ExampleExpandableDataProviderFragment";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main_list_exapand);
        if (savedInstanceState == null) {
            getSupportFragmentManager().beginTransaction()
                    .add(new ExampleExpandableDataProviderFragment(), FRAGMENT_TAG_DATA_PROVIDER)
                    .commit();
            getSupportFragmentManager().beginTransaction()
                    .add(R.id.container, DragDropListFragment.newInstance(), FRAGMENT_LIST_VIEW)
                    .commit();
        }
    }
    ...
    public void onGroupItemClicked(int groupPosition) {
        ...
        if (data.isPinned()) {
            ...
            ((DragDropListFragment) fragment).notifyGroupItemChanged(groupPosition);
        }
    }
    ...
    public void onChildItemClicked(int groupPosition, int childPosition) {
        ...
        if (data.isPinned()) {
            // unpin if tapped the pinned item
            data.setPinned(false);
            ((DragDropListFragment) fragment).notifyChildItemChanged(groupPosition, childPosition);
        }
    }
    ...
    private void onItemUndoActionClicked() {
        ...
        if (childPosition == RecyclerView.NO_POSITION) {
            // group item
            ((DragDropListFragment) fragment).notifyGroupItemRestored(groupPosition);
        } else {
            // child item
            ((DragDropListFragment) fragment).notifyChildItemRestored(groupPosition, childPosition);
        }
    }
